...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!secretKey) {
      return new Response(JSON.stringify({ error: 'Paychangu not configured' }), { status: 500, headers: corsHeaders });
    }

    // Get user profile for name
    const { data: profile } = await supabase.from('User').select('full_name, email').eq('id', user.id).single();
    ...
    if (data.status !== 'success') {
      return new Response(JSON.stringify({ error: data.message || 'Payment initiation failed' }), { status: 400, headers: corsHeaders });
    }
