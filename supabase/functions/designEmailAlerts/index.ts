...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!design_id || !event_type) {
      return new Response(JSON.stringify({ error: 'Missing design_id or event_type' }), {
    ...
    if (!config) {
      return new Response(JSON.stringify({ skipped: true, reason: `No email configured for status: ${event_type}` }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }

    const { data: design } = await admin.from('DesignRequest').select('*').eq('id', design_id).single()
    if (!design) {
    ...
    const { data: allUsers } = await admin.from('User').select('*')
    const user = allUsers?.find((u: any) => u.id === design.user_id)

    if (user?.email) {
      await sendEmail({ to: user.email, fromName: 'Brandfletch Designs', subject: config.subject, html: config.html(design, user) })
    }
    ...
    return new Response(JSON.stringify({ success: true, event_type, sent_to: user?.email }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    })
  } catch (err: any) {
