...
function PortfolioCard({ item, featured, onOpen }) {
  ...
  return (
    <div
      className={cn(
        'group rounded-xl overflow-hidden border border-border/60 bg-card hover:shadow-lg transition-all duration-300 cursor-pointer',
        featured && 'ring-2 ring-[hsl(var(--primary))]/20'
      )}
      onClick={() => onOpen({ item, imgIndex: 0 })}
    >
      {/* Image */}
...
// ── Lightbox ──────────────────────────────────────────────────────────────────
function Lightbox({ item, imgIndex, onClose }) {
  const [idx, setIdx] = useState(imgIndex);
  const allImages = [item.cover_image, ...(item.images || [])].filter(Boolean);
  ...
  return (
    ...
    >
      <div
        className="bg-card rounded-2xl overflow-hidden max-w-4xl w-full max-h-[90vh] flex flex-col md:flex-row shadow-2xl"
        onClick={e => e.stopPropagation()}
      >
        {/* Image side */}
        <div className="relative bg-muted flex-1 min-h-64">
          {allImages.length > 0 ? (
            <img src={allImages[idx]} alt={item.title} className="w-full h-full object-contain max-h-[60vh] md:max-h-none" />
          ) : (
        ...
        <div className="w-full md:w-80 p-6 flex flex-col overflow-y-auto">
          ...
          {item.description && <p className="text-sm text-muted-foreground mb-4 leading-relaxed">{item.description}</p>}
