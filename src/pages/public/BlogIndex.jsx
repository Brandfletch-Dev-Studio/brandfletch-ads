...
function PostCard({ post }) {
  ...
  return (
    <Link to={`/blog/${post.slug}`} className="group block bg-card border border-border rounded-2xl overflow-hidden hover:border-[hsl(var(--accent))]/40 hover:shadow-lg transition-all">
      {post.cover_image && (
      ...
      <div className="p-5">
        {post.category && (
          <Badge variant="outline" className="mb-2 text-[hsl(var(--accent))] border-[hsl(var(--accent))]/30 text-xs">{post.category}</Badge>
        )}
        <h3 className="font-bold text-foreground text-lg leading-tight mb-2 group-hover:text-[hsl(var(--accent))] transition-colors line-clamp-2">
          {post.title}
        </h3>
        <p className="text-muted-foreground text-sm leading-relaxed line-clamp-2 mb-4">{post.excerpt}</p>
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          <div className="flex items-center gap-3">
            <span className="flex items-center gap-1"><Calendar className="w-3 h-3" />{new Date(post.published_at || post.created_date).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'})}</span>
    ...
    <div>
      ...
      <section className="bg-[hsl(var(--primary))] text-white py-20 text-center">
        <div className="max-w-2xl mx-auto px-4">
          ...
          <h1 className="text-4xl sm:text-5xl font-bold font-display mb-4">The Brandfletch Blog</h1>
            ...
            <div className="flex gap-2 flex-wrap">
              {categories.map(c => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${category === c ? 'bg-[hsl(var(--accent))] text-white' : 'bg-secondary text-foreground hover:bg-secondary/70'}`}
                >
                  {c}
                </button>
              ))}
